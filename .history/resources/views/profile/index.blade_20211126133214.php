    ...
    <div class="container p-3 my-3 border d-flex flex-wrap">
        ...
        <div class="profile-info ms-5" >
            <h1 class="position-relative">{{ $user->name }} 
                @if ($user->is_member == 0)
                <span class="badge bg-secondary p-2 fs-6 position-absolute top-0 start-100 ms-1"><a href="/profile/member" class="text-white text-decoration-none">Upgrade to Member</a></span>
                @else
                <span class="badge bg-warning p-2 fs-6 position-absolute top-0 start-100 ms-1"></span>
                @endif
            </h1>
