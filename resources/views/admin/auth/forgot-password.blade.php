...
<body>
    <div class="w-full bg-white ">
        <h1 class="text-2xl w-full font-medium text-blue-500 shadow p-3 fixed">Forgot password admin</h1>
        <form action="" method="POST" class=" p-3 w-full  flex justify-center h-screen items-center">
            ...
            <div class="border w-11/12 md:w-2/5 lg:w-2/5 xl:2/5 sm:w-2/5 rounded p-3 shadow-lg">
                @if (session()->has('errorVerification'))
                    <div id="alert-2" class="flex w-full p-4 mb-4 bg-red-100 rounded-lg dark:bg-red-200" role="alert">
                        <svg class="flex-shrink-0 w-5 h-5 text-red-700 dark:text-red-800" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path></svg>
                        <div class="ml-3 text-sm font-medium text-red-700 dark:text-red-800">
                            {{session('errorVerification')}}
                        </div>
                ...
                @endif
                @if (session()->has('alreadyAvailable'))
                ...
                @endif
                <p class="text-center font-medium mb-2">Masukkan email terakhir yang anda ingat!</p><hr>
                <div class="w-full mt-2">
                    <label for="" class="block font-medium ml-2">Email</label>
                    <input type="text" name="email" class="block p-2 w-full border border-2 rounded @error('email') border-red-500 border-2 @else border-gray-300 @enderror">
                    @error('email')
                        <p class="text-red-500 text-sm font-medium">{{ $message }}</p>
                    @enderror
                </div>
                <div class="w-full mt-2">
                    <button type="submit" class="p-2 w-full text-white font-medium rounded bg-green-500 hover:bg-green-600 hover:shadow">Send</button>
                </div>
            </div>
        </form>
    </div>
